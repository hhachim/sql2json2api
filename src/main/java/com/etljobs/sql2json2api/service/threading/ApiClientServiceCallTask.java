...
/**
 * Implémentation concrète de ApiCallTask qui délègue l'exécution au service
 * ApiClientService existant.
 */
@Slf4j
public class ApiClientServiceCallTask extends ApiCallTask {
    ...
    protected ApiResponse executeApiCall() throws Exception {
        log.debug("[{}] Délégation de l'appel API à ApiClientService: {} {}",
                correlationId, getMethod(), getUrl());
        ...
        try {
            ...
            if (legacyResponse != null) {
                log.debug("[{}] Réponse API reçue: statut={}, taille corps={}",
                        correlationId, legacyResponse.getStatusCode(),
                ...
                if (legacyResponse.getBody() != null && legacyResponse.getBody().length() < 1000) {
                    log.debug("[{}] Corps de la réponse: {}", correlationId, legacyResponse.getBody());
                }
            } else {
                log.warn("[{}] Réponse API nulle reçue", correlationId);
            }
            ...
            log.error("[{}] Erreur lors de l'appel API à ApiClientService: {}",
